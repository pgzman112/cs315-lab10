...
void buildTree(string fileName, BinarySearchTree<string>* myBst);

int main(int argc, char* argv[]){
	// Do many things here
	string fileName = argv[1];
	BinarySearchTree<string> * myBst = new BinarySearchTree<string>();
	buildTree(fileName, myBst);
	...
	while(true){
		...
		if(userInput < 1 || userInput > 4){
			...
			vector<int> lineNums;
			if(myBst->contains(in, lineNums) == true){
...
void buildTree(string fileName, BinarySearchTree<string>* myBst){
	...
	cout << "Total number of distinct words: " << myBst->countTreeNodes() << endl;
	cout << "Time taken to build index using BST was: " << time_span.count() << " seconds" << endl;
	cout << "Heigh of the Binary Search Tree is: " << myBst->Treeheight() << endl;
